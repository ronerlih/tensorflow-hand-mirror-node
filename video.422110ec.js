...
/**
    ...
    * =============================================================================
    */
function getBoxSize(t) {
  return [Math.abs(t.endPoint[0] - t.startPoint[0]), Math.abs(t.endPoint[1] - t.startPoint[1])];
}

function getBoxCenter(t) {
  return [t.startPoint[0] + (t.endPoint[0] - t.startPoint[0]) / 2, t.startPoint[1] + (t.endPoint[1] - t.startPoint[1]) / 2];
}

function cutBoxFromImageAndResize(t, n, e) {
  const o = n.shape[1],
        s = n.shape[2],
        i = [[t.startPoint[1] / o, t.startPoint[0] / s, t.endPoint[1] / o, t.endPoint[0] / s]];
  return _tfjsCore.image.cropAndResize(n, i, [0], e);
}

function scaleBoxCoordinates(t, n) {
  return {
    startPoint: [t.startPoint[0] * n[0], t.startPoint[1] * n[1]],
    endPoint: [t.endPoint[0] * n[0], t.endPoint[1] * n[1]],
    palmLandmarks: t.palmLandmarks.map(t => {
      return [t[0] * n[0], t[1] * n[1]];
    })
  };
}

function enlargeBox(t, n = 1.5) {
  const e = getBoxCenter(t),
        o = getBoxSize(t),
        s = [n * o[0] / 2, n * o[1] / 2];
  return {
...
function squarifyBox(t) {
  const n = getBoxCenter(t),
        e = getBoxSize(t),
        o = Math.max(...e) / 2;
  return {
    startPoint: [n[0] - o, n[1] - o],
    endPoint: [n[0] + o, n[1] + o],
    palmLandmarks: t.palmLandmarks
  };
}

function shiftBox(t, n) {
...
class HandDetector {
  constructor(t, n, e, o, s, i) {
    this.model = t, this.width = n, this.height = e, this.iouThreshold = s, this.scoreThreshold = i, this.anchors = o.map(t => [t.x_center, t.y_center]), this.anchorsTensor = (0, _tfjsCore.tensor2d)(this.anchors), this.inputSizeTensor = (0, _tfjsCore.tensor1d)([n, e]), this.doubleInputSizeTensor = (0, _tfjsCore.tensor1d)([2 * n, 2 * e]);
  }

  normalizeBoxes(t) {
    return (0, _tfjsCore.tidy)(() => {
      const n = (0, _tfjsCore.slice)(t, [0, 0], [-1, 2]),
            e = (0, _tfjsCore.slice)(t, [0, 2], [-1, 2]),
            o = (0, _tfjsCore.add)((0, _tfjsCore.div)(n, this.inputSizeTensor), this.anchorsTensor),
            s = (0, _tfjsCore.div)(e, this.doubleInputSizeTensor),
            i = (0, _tfjsCore.mul)((0, _tfjsCore.sub)(o, s), this.inputSizeTensor),
            r = (0, _tfjsCore.mul)((0, _tfjsCore.add)(o, s), this.inputSizeTensor);
      return (0, _tfjsCore.concat2d)([i, r], 1);
    });
  }

  normalizeLandmarks(t, n) {
    return (0, _tfjsCore.tidy)(() => {
      const e = (0, _tfjsCore.add)((0, _tfjsCore.div)(t.reshape([-1, 7, 2]), this.inputSizeTensor), this.anchors[n]);
      return (0, _tfjsCore.mul)(e, this.inputSizeTensor);
    });
  }
  ...
  estimateHandBounds(t) {
    ...
    const i = s.boxes.arraySync(),
          r = i[0].slice(0, 2),
          a = i[0].slice(2, 4),
          d = s.palmLandmarks.arraySync();
    return o.dispose(), s.boxes.dispose(), s.palmLandmarks.dispose(), scaleBoxCoordinates({
      startPoint: r,
      endPoint: a,
      palmLandmarks: d
...
function normalizeRadians(t) {
  return t - 2 * Math.PI * Math.floor((t + Math.PI) / (2 * Math.PI));
}

function computeRotation(t, n) {
  return normalizeRadians(Math.PI / 2 - Math.atan2(-(n[1] - t[1]), n[0] - t[0]));
}

const buildTranslationMatrix = (t, n) => [[1, 0, t], [0, 1, n], [0, 0, 1]];

function dot(t, n) {
  let e = 0;

  for (let o = 0; o < t.length; o++) e += t[o] * n[o];

  return e;
}

function getColumnFrom2DArr(t, n) {
  const e = [];

  for (let o = 0; o < t.length; o++) e.push(t[o][n]);

  return e;
}

function multiplyTransformMatrices(t, n) {
  const e = [],
        o = t.length;
  ...
  for (let s = 0; s < o; s++) {
    ...
    for (let i = 0; i < o; i++) e[s].push(dot(t[s], getColumnFrom2DArr(n, i)));
  }

  return e;
}

function buildRotationMatrix(t, n) {
  const e = Math.cos(t),
        o = Math.sin(t),
        s = [[e, -o, 0], [o, e, 0], [0, 0, 1]];
  return multiplyTransformMatrices(multiplyTransformMatrices(buildTranslationMatrix(n[0], n[1]), s), buildTranslationMatrix(-n[0], -n[1]));
}

function invertTransformMatrix(t) {
  const n = [[t[0][0], t[1][0]], [t[0][1], t[1][1]]],
        e = [t[0][2], t[1][2]],
        o = [-dot(n[0], e), -dot(n[1], e)];
  return [n[0].concat(o[0]), n[1].concat(o[1]), [0, 0, 1]];
}

function rotatePoint(t, n) {
  return [dot(t, n[0]), dot(t, n[1])];
}

const UPDATE_REGION_OF_INTEREST_IOU_THRESHOLD = .8,
      PALM_BOX_SHIFT_VECTOR = [0, -.4],
      PALM_BOX_ENLARGE_FACTOR = 3,
      HAND_BOX_SHIFT_VECTOR = [0, -.1],
...
class HandPipeline {
  constructor(t, n, e, o, s, i) {
    this.regionsOfInterest = [], this.runsWithoutHandDetector = 0, this.boundingBoxDetector = t, this.meshDetector = n, this.maxContinuousChecks = s, this.detectionConfidence = i, this.meshWidth = e, this.meshHeight = o, this.maxHandsNumber = 1;
  }

  getBoxForPalmLandmarks(t, n) {
    const e = t.map(t => {
      return rotatePoint([...t, 1], n);
    });
    return enlargeBox(squarifyBox(shiftBox(this.calculateLandmarksBoundingBox(e), PALM_BOX_SHIFT_VECTOR)), PALM_BOX_ENLARGE_FACTOR);
  }

  getBoxForHandLandmarks(t) {
    const n = enlargeBox(squarifyBox(shiftBox(this.calculateLandmarksBoundingBox(t), HAND_BOX_SHIFT_VECTOR)), HAND_BOX_ENLARGE_FACTOR),
          e = [];

    for (let n = 0; n < PALM_LANDMARK_IDS.length; n++) e.push(t[PALM_LANDMARK_IDS[n]].slice(0, 2));

    return n.palmLandmarks = e, n;
  }

  transformRawCoords(t, n, e, o) {
    const s = getBoxSize(n),
          i = [s[0] / this.meshWidth, s[1] / this.meshHeight],
          r = t.map(t => [i[0] * (t[0] - this.meshWidth / 2), i[1] * (t[1] - this.meshHeight / 2), t[2]]),
          a = buildRotationMatrix(e, [0, 0]),
          d = r.map(t => {
  ...
  async estimateHand(t) {
    ...
    const e = this.regionsOfInterest[0],
          o = computeRotation(e.palmLandmarks[PALM_LANDMARKS_INDEX_OF_PALM_BASE], e.palmLandmarks[PALM_LANDMARKS_INDEX_OF_MIDDLE_FINGER_BASE]),
          s = getBoxCenter(e),
    ...
    const _ = (0, _tfjsCore.reshape)(p, [-1, 3]),
          P = _.arraySync();
    ...
    return this.updateRegionsOfInterest(M, !1), {
      landmarks: g,
      handInViewConfidence: f,
      boundingBox: {
        topLeft: M.startPoint,
        bottomRight: M.endPoint
      }
    };
  }
...
async function loadHandDetectorModel() {
  return (0, _tfjsConverter.loadGraphModel)("https://tfhub.dev/mediapipe/tfjs-model/handdetector/1/default/1", {
    fromTFHub: !0
  });
}

const MESH_MODEL_INPUT_WIDTH = 256,
      MESH_MODEL_INPUT_HEIGHT = 256;

async function loadHandPoseModel() {
  return (0, _tfjsConverter.loadGraphModel)("https://tfhub.dev/mediapipe/tfjs-model/handskeleton/1/default/1", {
    fromTFHub: !0
  });
}

async function loadAnchors() {
  return _tfjsCore.util.fetch("https://tfhub.dev/mediapipe/tfjs-model/handskeleton/1/default/1/anchors.json?tfjs-format=file").then(t => t.json());
}

async function load({
  maxContinuousChecks: t = 1 / 0,
  detectionConfidence: n = .8,
  iouThreshold: e = .3,
  scoreThreshold: o = .5
} = {}) {
  const [s, i, r] = await Promise.all([loadAnchors(), loadHandDetectorModel(), loadHandPoseModel()]),
        a = new HandDetector(i, MESH_MODEL_INPUT_WIDTH, MESH_MODEL_INPUT_HEIGHT, s, e, o),
        d = new HandPipeline(a, r, MESH_MODEL_INPUT_WIDTH, MESH_MODEL_INPUT_HEIGHT, t, n);
  return new HandPose(d);
}
...
function flipHandHorizontal(t, n) {
  const {
...
class HandPose {
  ...
  async estimateHands(t, n = !1) {
    const [, e] = getInputTensorDimensions(t),
          o = (0, _tfjsCore.tidy)(() => (t instanceof _tfjsCore.Tensor || (t = _tfjsCore.browser.fromPixels(t)), t.toFloat().expandDims(0))),
          s = await this.pipeline.estimateHand(o);
    if (o.dispose(), null === s) return [];
    let i = s;
    !0 === n && (i = flipHandHorizontal(s, e));
    const r = {};

    for (const t of Object.keys(MESH_ANNOTATIONS)) r[t] = MESH_ANNOTATIONS[t].map(t => i.landmarks[t]);

    return [{
      handInViewConfidence: i.handInViewConfidence,
...
},{}],"node_modules/validate.io-array/lib/index.js":[function(require,module,exports) {
'use strict';
...
},{}],"node_modules/validate.io-function/lib/index.js":[function(require,module,exports) {
/**
*
*	VALIDATE: function
*
...
},{}],"node_modules/compute-dot/lib/index.js":[function(require,module,exports) {
'use strict';
...
},{"validate.io-array":"node_modules/validate.io-array/lib/index.js","validate.io-function":"node_modules/validate.io-function/lib/index.js"}],"node_modules/compute-cosine-similarity/lib/index.js":[function(require,module,exports) {
'use strict';
...
},{}],"utils/findSimilarVideo.js":[function(require,module,exports) {
"use strict";
...
exports.buildImagesData = buildImagesData;
...
async function findMatch(lookupPose) {
  const similarIndexOrder = await (0, _findSimilarVideo.findSimilar)(lookupPose);
...
function drawToCanvas(canvas, predictions, img, drawHandPoseOnData) {
  ...
  if (predictions.length > 0 && drawHandPoseOnData) {
    ...
    context.rect(predictions[0].boundingBox.topLeft[0] > 0 ? predictions[0].boundingBox.topLeft[0] : 0, predictions[0].boundingBox.topLeft[1] > 0 ? predictions[0].boundingBox.topLeft[1] : 0, predictions[0].boundingBox.bottomRight[0] - predictions[0].boundingBox.topLeft[0] < 300 ? predictions[0].boundingBox.bottomRight[0] - predictions[0].boundingBox.topLeft[0] : 300, predictions[0].boundingBox.bottomRight[1] - predictions[0].boundingBox.topLeft[1] < 300 ? predictions[0].boundingBox.bottomRight[1] - predictions[0].boundingBox.topLeft[1] : 300);
    context.stroke();
  }
}
...
async function placeImage(ctx, prediction, matchIndex, options) {
  ...
  if (options.difference) ctx.globalCompositeOperation = 'difference'; // drawPoint(ctx,0,0,5)
